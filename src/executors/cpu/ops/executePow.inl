void CpuExecutor::executePow(const Node& node) {
    // Pow: Y = A ^ B
    ...
    if (B_is_scalar || (A->shape() == B->shape())) {
        ...
        if (B_is_scalar) {
            ...
            for (int64_t i = 0; i < size; ++i) {
                output->data_ptr<float>()[i] = std::pow(A->data_ptr<float>()[i], exponent);
            }
        } else {
            powCPU(A->data_ptr<float>(), B->data_ptr<float>(), output->data_ptr<float>(), size, 1);
        }
