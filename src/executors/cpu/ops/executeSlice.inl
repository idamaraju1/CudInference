void CpuExecutor::executeSlice(const Node& node) {
    ...
    if (node.inputs().size() >= 3) {
        ...
        if (starts_tensor->dtype() == DataType::INT64) {
            const int64_t* starts_data = starts_tensor->data_ptr<int64_t>();
            const int64_t* ends_data = ends_tensor->data_ptr<int64_t>();
            ...
            const float* starts_data = starts_tensor->data_ptr<float>();
            const float* ends_data = ends_tensor->data_ptr<float>();
            for (size_t i = 0; i < starts_tensor->size(); ++i) {
        ...
        if (node.inputs().size() >= 5) {
            ...
            if (steps_tensor->dtype() == DataType::INT64) {
                const int64_t* steps_data = steps_tensor->data_ptr<int64_t>();
                for (size_t i = 0; i < steps_tensor->size(); ++i) {
                    steps.push_back(steps_data[i]);
                }
            } else {
                const float* steps_data = steps_tensor->data_ptr<float>();
                for (size_t i = 0; i < steps_tensor->size(); ++i) {
    ...
    // Execute on CPU
    sliceCPU(input->data_ptr<float>(), output->data_ptr<float>(), input->shape(), full_starts, full_steps, output_shape, 1);
