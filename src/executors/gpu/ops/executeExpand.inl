void GpuExecutor::executeExpand(const Node& node) {
    ...
    // TODO - refactor: GPU Expand kernel not yet implemented
    // For now, compute on CPU then copy to GPU
    switch (input->dtype()) {
        case DataType::FLOAT32: {
            std::vector<uint8_t> cache;
            const float* src = getHostData<float>(input, cache);
            auto output_cpu = std::make_shared<CpuTensor>(output_shape, DataType::FLOAT32);
            broadcastCopy(src, output_cpu->data_ptr<float>(), input->shape(), output_shape);
        ...
        case DataType::INT32: {
            ...
            auto output_cpu = std::make_shared<CpuTensor>(output_shape, DataType::INT32);
            broadcastCopy(src, output_cpu->data_ptr<int32_t>(), input->shape(), output_shape);
            auto output = output_cpu->toGPU();
            tensors_[node.outputs()[0]] = output;
            break;
        }
        case DataType::INT64: {
            ...
            auto output_cpu = std::make_shared<CpuTensor>(output_shape, DataType::INT64);
            broadcastCopy(src, output_cpu->data_ptr<int64_t>(), input->shape(), output_shape);
            auto output = output_cpu->toGPU();
            tensors_[node.outputs()[0]] = output;
            break;
        }
        case DataType::UINT8: {
            ...
            broadcastCopy(src, output_cpu->data_ptr<uint8_t>(), input->shape(), output_shape);
            auto output = output_cpu->toGPU();
            tensors_[node.outputs()[0]] = output;
            break;
        }
