void GpuExecutor::executePow(const Node& node) {
    ...
    if (B_is_scalar || (A->shape() == B->shape())) {
        ...
        if (B_is_scalar) {
            ...
            launchPowScalarKernel(A->data_ptr<float>(), exponent, output->data_ptr<float>(), size);
        } else {
            launchPowKernel(A->data_ptr<float>(), B->data_ptr<float>(), output->data_ptr<float>(), size);
        }
