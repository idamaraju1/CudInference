void GpuExecutor::executeScatterND(const Node& node) {
    ...
    // TODO - refactor: GPU ScatterND kernel not yet implemented
    // For now, compute on CPU then copy to GPU
    auto output_cpu = std::make_shared<CpuTensor>(data->shape(), DataType::FLOAT32);
    std::vector<uint8_t> data_cache;
    const float* data_ptr = getHostData<float>(data, data_cache);
    std::memcpy(output_cpu->data_ptr<float>(), data_ptr, data->size() * sizeof(float));
