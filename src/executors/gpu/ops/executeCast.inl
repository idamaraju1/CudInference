void GpuExecutor::executeCast(const Node& node) {
    ...
    // TODO - refactor: GPU Cast kernel not yet implemented
    // For now, compute on CPU then copy to GPU
    auto output_cpu = std::make_shared<CpuTensor>(input->shape(), target_dtype);
    ...
    switch (input->dtype()) {
        case DataType::FLOAT32: {
            ...
            dispatchCastToTarget(src, target_dtype, output_cpu, count);
            break;
        }
        case DataType::INT32: {
