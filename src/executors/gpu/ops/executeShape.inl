void GpuExecutor::executeShape(const Node& node) {
    ...
    int64_t* data_ptr = output_cpu->data_ptr<int64_t>();
    for (size_t i = 0; i < input_shape.size(); ++i) {
        data_ptr[i] = input_shape[i];
    }

    // TODO - refactor: Shape operation doesn't need GPU kernel (metadata only)
    // But we still need to transfer to GPU for consistency
