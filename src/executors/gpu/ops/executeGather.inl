void GpuExecutor::executeGather(const Node& node) {
    ...
    // Copy indices to GPU
    int64_t* indices_gpu;
    CUDA_CHECK(cudaMalloc(&indices_gpu, host_indices.size() * sizeof(int64_t)));
    CUDA_CHECK(cudaMemcpy(indices_gpu, host_indices.data(), host_indices.size() * sizeof(int64_t), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaFree(indices_gpu));
