void GpuExecutor::executeReduceSum(const Node& node) {
    ...
    if (axes.empty() && node.inputs().size() >= 2) {
        ...
        if (axes_tensor->dtype() == DataType::INT64) {
            const int64_t* axis_data = axes_tensor->data_ptr<int64_t>();
            for (size_t i = 0; i < axes_tensor->size(); ++i) {
                axes.push_back(axis_data[i]);
            }
        } else {
            const float* axis_data = axes_tensor->data_ptr<float>();
            for (size_t i = 0; i < axes_tensor->size(); ++i) {
    ...
    if (use_cpu_fallback_) {
        std::memcpy(output->data_ptr<float>(), host_output.data(), bytes);
    } else {
        CUDA_CHECK(cudaMemcpy(output->data_ptr<float>(), host_output.data(), bytes, cudaMemcpyHostToDevice));
    }
