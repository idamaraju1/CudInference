void GpuExecutor::executeGemm(const Node& node) {
    ...
    if (transA) {
        ...
        transposeMatrix(A_temp->data_ptr<float>(), A_op->data_ptr<float>(), A->dim(0), A->dim(1), use_cpu_fallback_);
    ...
    if (transB) {
        ...
        transposeMatrix(B_temp->data_ptr<float>(), B_op->data_ptr<float>(), B->dim(0), B->dim(1), use_cpu_fallback_);
